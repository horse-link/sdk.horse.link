...
test.concurrent.each([
  ...
  ["3456.3456", "3456.3456"],
  ["1.00000", "1.0000"]
])("should have trailing zero", async (input: string, expected: string): Promise<void> => {
  const result = formatting.formatToFourDecimals(input);
...
])("round to four decimal", async (input: string, expected: string): Promise<void> => {
  const result = formatting.formatToFourDecimals(input);
  expect(result).toBe(expected);
});
...
test.concurrent.each([
  ...
  "0.000099",
  "0.000005132156"
])("less than 0.0001", async (input: string): Promise<void> => {
  const result = formatting.formatToFourDecimals(input);
...
test.concurrent.each([
  ...
  ["100.1045", "100.10"],
  ["1000.2895", "1000.29"]
])("round to two decimals", async (input: string, expected: string): Promise<void> => {
  const result = formatting.formatToTwoDecimals(input);
  expect(result).toBe(expected);
});

describe("address tests", () => {
  it("should shorten address", () => {
    const actual = formatting.shortenAddress(
  ...
  it("should shorten hash", () => {
    const actual = formatting.shortenHash(
